/**
 * NeonType Race — Cross-Network Multiplayer Fix
 *
 * ROOT CAUSE of "works on same PC, fails on different PC":
 * ─────────────────────────────────────────────────────────
 * Different PCs on different home networks are behind NAT routers.
 * STUN servers only tell each peer their public IP — they can't
 * punch through strict NAT firewalls. TURN servers RELAY the data
 * through a middle server, bypassing NAT completely.
 *
 * This version uses Metered.ca free TURN credentials (no signup)
 * + multiple fallback TURN servers to guarantee connectivity.
 */
...
const JOIN_TIMEOUT   = 25000; // 25s — TURN relay negotiation takes time

// ── ICE / TURN config ────────────────────────────────────────────────────────
// Using multiple free TURN providers as fallbacks.
// TURN is the only thing that makes cross-network WebRTC reliable.
const ICE_SERVERS = [
    // STUN — fast, free, discover public IP
    ...
    // TURN via Open Relay Project — free, no auth, reliable
    { urls: 'turn:openrelay.metered.ca:80',              username: 'openrelayproject', credential: 'openrelayproject' },
    { urls: 'turn:openrelay.metered.ca:443',             username: 'openrelayproject', credential: 'openrelayproject' },
    { urls: 'turn:openrelay.metered.ca:443?transport=tcp', username: 'openrelayproject', credential: 'openrelayproject' },
    { urls: 'turns:openrelay.metered.ca:443',            username: 'openrelayproject', credential: 'openrelayproject' },

    // TURN via Numb (backup)
    { urls: 'turn:numb.viagenie.ca',                     username: 'webrtc@live.com',  credential: 'muazkh'           },

    // TURN via FreeTURN (backup)
...
function makePeerConfig() {
    return {
        ...
        config: {
            ...
            iceCandidatePoolSize: 10,    // pre-gather candidates = faster connect
...
// ── Room / Peer IDs ──────────────────────────────────────────────────────────
// Host peer ID = 'r-' + roomCode  (e.g. 'r-bxmk')
// Guest peer ID = 'g-' + timestamp+random (unique every time)
// Room codes = 4 lowercase letters, no ambiguous chars (l, o)
...
function hostPeerId(code)  { return 'r-' + code.toLowerCase(); }
function newGuestPeerId()  { return 'g-' + Date.now().toString(36) + Math.random().toString(36).slice(2,6); }
...
const TEXTS = [
    ...
    `Rain has a way of erasing the boundaries between things. Puddles reflect neon signs in shattered fragments, making the wet street look like a mosaic of broken light. Pedestrians become silhouettes with glowing edges, their umbrellas like black mushrooms pushing through a luminous underworld. The rhythm of the rain on the pavement is the city heartbeat, steady and relentless, punctuated by the hiss of tires and the distant thunder that rolls between towers. On nights like this, even the most hardened city dweller pauses, looks up, and remembers they are beautifully small.`,
    `Typing is a skill that rewards patience above all else. The fingers that move fastest are rarely the ones that started fastest. They are the ones that built their speed slowly and deliberately through thousands of hours of careful repetition. Every character is a small decision about which finger, which motion, which fraction of a second to use. Speed emerges from the elimination of hesitation, and hesitation fades only when the body knows the keyboard so well that thought and motion become the same event. Practice is mastery, one keystroke at a time.`,
    `Space is not empty. Between the visible stars, threads of gas and magnetic fields connect everything in a vast invisible web. Galaxies pull on each other across distances too great to comprehend, their influence travelling at the speed of gravity itself. Black holes anchor the centres of almost every galaxy we have studied, spinning slowly like cosmic engines powering the structure of the universe. Light that left distant stars before Earth formed is only now reaching our telescopes, carrying ancient information about things long transformed.`,
    `Networks are not built from cables alone. They are built from trust, the quiet agreement between machines that the data passing between them is worth protecting and delivering intact. Every packet carries a tiny flag that says it came from somewhere real and is going somewhere real. The engineers who designed these protocols in sparse offices never imagined the volume that would one day flow through their inventions. Yet the architecture holds, because it was built on sound principles: redundancy, error correction, and the assumption that every message must get through.`,
    ...
    `The archive was three floors underground, temperature-controlled to preserve paper older than any living institution. Rows of grey metal shelves stretched into darkness beyond the reach of portable lights the researcher carried through narrow corridors. Every box held a fragment of a world that had believed itself permanent. Letters, receipts, maps, census records, each one a small act of preservation by someone who had sensed that details would matter to the future. The researcher moved carefully, gloves on, reading a world that had not expected to be read again.`
];
...
function broadcastAll(data)         { Object.values(guestConns).forEach(c => send(c, data)); }
function broadcastExcept(skip, data){ Object.entries(guestConns).forEach(([id,c]) => { if(id!==skip) send(c,data); }); }

// ── Utilities ────────────────────────────────────────────────────────────────
function esc(s)       { return String(s||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;'); }
function calcLimit(t) { return Math.ceil(t.trim().split(/\s+/).length / IDEAL_WPM * 60 * TIME_FACTOR); }
function fmtTime(s)   { s=Math.max(0,Math.floor(s)); return Math.floor(s/60)>0?`${Math.floor(s/60)}:${(s%60).toString().padStart(2,'0')}`:`${s}s`; }
...
// Status bar shown in lobby — with animated dots while connecting
function setStatus(msg, type='info') {
    const el = $('lobby-status'); if(!el) return;
    el.textContent = msg;
    el.className   = 'lobby-status-bar status-' + type;
}
...
function initLanding() {
    ...
    $('input-player-name').addEventListener('input', function(){ profile.name=this.value.trim()||'Racer'; saveProfile(); });
    buildCars(); buildColors();
}

function buildCars() {
    const row=$('quick-car-row'); if(!row) return; row.innerHTML='';
    CARS.forEach(car=>{
...
function buildColors() {
    const w=$('quick-colors'); if(!w) return; w.innerHTML='';
    QUICK_COLORS.forEach(col=>{
        const s=document.createElement('div');
        s.className='qcolor'+(col===profile.carColor?' selected':'');
        s.style.background=col;
        s.onclick=()=>{ profile.carColor=col; saveProfile(); document.querySelectorAll('.qcolor').forEach(x=>x.classList.remove('selected')); s.classList.add('selected'); buildCars(); };
        w.appendChild(s);
    });
}
...
function renderLobby() {
    const grid=$('lobby-players'); if(!grid) return; grid.innerHTML='';
    ...
    if(isHost){
        if(active>=2){ sb.classList.remove('hidden'); if(hint)hint.style.display='none'; }
        else{ sb.classList.add('hidden'); if(hint){hint.style.display='block'; hint.textContent=`Need ${2-active} more player(s) to start...`;} }
        setStatus(`Room ready · ${active}/${MAX_PLAYERS} Players`, 'ok');
    } else {
        setStatus(`Connected · ${active}/${MAX_PLAYERS} Players`, 'ok');
    }
}

// ── Race tracks ──────────────────────────────────────────────────────────────
function renderTracks() {
    const c=$('tracks-container'); if(!c) return; c.innerHTML='';
    players.filter(Boolean).forEach(p=>{
        const isMe=p.slotIndex===mySlotIndex, row=document.createElement('div');
...
function updateRing(rem) {
    const fill=$('time-ring-fill'), num=$('time-remaining'); if(!fill||!num) return;
...
function onTimeUp(){
    ...
    updateTracks(); showResult(p.finishRank,p.wpm,p.accuracy,true);
}
function checkDone(){
...
function startCountdown(cb){
    const ov=$('countdown-overlay'), num=$('countdown-number'); if(!ov||!num){cb();return;}
    ov.classList.remove('hidden'); let n=COUNTDOWN_SECS;
...
function prepareRace(text,allPlayers,mySlot){
    ...
    $('hud-room').textContent=roomCode.toUpperCase();
    ...
    const inp=$('typing-input'); if(inp){inp.value='';inp.disabled=true;inp.placeholder='Get ready...';}
    showView('race');
    startCountdown(()=>{
        startTime=Date.now();
...
function onEndRace(fp){
    ...
    clearInterval(raceTimer);
    const p=me();
    if(p&&!p.finished){p.finished=true;p.timedOut=true;const inp=$('typing-input');if(inp)inp.disabled=true;showResult(players.filter(Boolean).length,p.wpm,p.accuracy,true);}
    updateTracks();
}
...
function showResult(rank,wpm,accuracy,timedOut){
    ...
    $('result-leaderboard').innerHTML=sorted.map(p=>{
        ...
        return `<div class="lb-row ${p.slotIndex===mySlotIndex?'lb-me':''}" style="--p-color:${p.color}"><span class="lb-rank">${ords[(p.finishRank||4)-1]||p.finishRank+'th'}</span><span class="lb-car">${p.emoji}</span><span class="lb-name">${esc(p.name)}</span><span class="lb-wpm">${p.wpm} WPM</span><span class="lb-status ${st}">${stTxt}</span></div>`;
    }).join('');
...
function lockBtns(on){
    ['btn-host','btn-join'].forEach(id=>{const b=$(id);if(!b)return;b.disabled=on;b.style.opacity=on?'0.5':'';});
}

// ── Connection status banner ─────────────────────────────────────────────────
// Shows animated dots while connecting so users know it's working
let dotTimer=null;
function showConnecting(msg){
    const el=$('lobby-status'); if(!el) return;
    let dots=0;
    clearInterval(dotTimer);
    el.className='lobby-status-bar status-connecting';
    dotTimer=setInterval(()=>{ dots=(dots+1)%4; el.textContent=msg+'.'.repeat(dots); },500);
    el.textContent=msg;
}
function stopConnecting(){ clearInterval(dotTimer); }
...
function hostGame(){
    ...
    guestConns={}; guestSlots={}; nextSlot=1; hostRetries=0;
...
function _startHostPeer(){
    const pid=hostPeerId(roomCode);
    console.log('[Host] Registering peer ID:', pid);

    peer=new Peer(pid, makePeerConfig());

    peer.on('open', id=>{
        console.log('[Host] ✅ Open, ID:', id);
        stopConnecting();
        $('display-room-code').textContent=roomCode.toUpperCase();
        $('hud-room').textContent=roomCode.toUpperCase();
    ...
    peer.on('connection', conn=>{
        ...
        conn.on('open',()=>{
            console.log('[Host] ✅ Guest connected, slot:', slot);
            send(conn,{type:'WELCOME',slotIndex:slot,players:players.map(p=>p?{...p}:null),roomCode:roomCode.toUpperCase()});
    ...
    // Auto-reconnect to signaling server if dropped
    peer.on('disconnected',()=>{
        console.warn('[Host] Signaling server disconnected, reconnecting...');
        try{ peer.reconnect(); }catch(e){}
    });

    peer.on('error', err=>{
        console.error('[Host] Peer error:', err.type, err);
        lockBtns(false); stopConnecting();
        if(err.type==='unavailable-id'){
            if(hostRetries<5){ hostRetries++; roomCode=makeRoomCode(); setTimeout(_startHostPeer,400); }
            else{ alert('Could not create room. Please refresh and try again.'); resetGame(); }
        } else if(['network','server-error','socket-error','socket-closed'].includes(err.type)){
            alert(`Network error (${err.type}). Check your internet and try again.`);
            resetGame();
        } else {
    ...
    switch(d.type){
        case 'IDENTIFY':
            if(slot!==undefined&&players[slot]){
                ...
                send(guestConns[pid],{type:'WELCOME',slotIndex:slot,players:players.map(p=>p?{...p}:null),roomCode:roomCode.toUpperCase()});
...
function joinGame(code){
    ...
    const gid=newGuestPeerId();
    console.log('[Guest] Creating peer:', gid, '| Joining room:', code);

    peer=new Peer(gid, makePeerConfig());

    peer.on('open', id=>{
        console.log('[Guest] ✅ Peer open:', id, '— connecting to host...');
        gotoGame(); showView('lobby');
        $('display-room-code').textContent=code.toUpperCase();
        showConnecting('Connecting to host');
        lockBtns(false);

        // Show a friendly timeout with clear instructions
        joinTimer=setTimeout(()=>{
            if(mySlotIndex===-1){
                stopConnecting();
                alert(
                    `⚠️ Could not connect to room "${code.toUpperCase()}"\n\n`+
                    `This usually means:\n`+
                    `  • The room code is wrong (check carefully)\n`+
                    `  • The host's tab was closed\n`+
                    `  • A firewall is blocking the connection\n\n`+
                    `Try: Ask the host to create a NEW room and share the new code.`
                );
                resetGame();
            }
        }, JOIN_TIMEOUT);

        const targetId=hostPeerId(code);
        console.log('[Guest] Connecting to peer ID:', targetId);
        ...
        conn.on('open',()=>{
            ...
            clearTimeout(joinTimer); stopConnecting();
        ...
        conn.on('close', ()=>{
            clearTimeout(joinTimer); stopConnecting();
            if(mySlotIndex!==-1){ alert('Host disconnected from the room.'); resetGame(); }
        });

        conn.on('error', err=>{
            console.error('[Guest] conn error:',err);
            clearTimeout(joinTimer); stopConnecting();
            alert('Lost connection to host. Please try joining again.');
            resetGame();
        });
    });
    ...
    peer.on('error', err=>{
        ...
        clearTimeout(joinTimer); stopConnecting(); lockBtns(false);
        if(err.type==='peer-unavailable'){
            alert(
                `❌ Room "${code.toUpperCase()}" not found.\n\n`+
                `Check that:\n`+
                `  ✓ The code is correct (4 letters, no numbers)\n`+
                `  ✓ The host's tab is still open\n`+
                `  ✓ You're both using: ${location.hostname}\n\n`+
                `Ask the host to create a new room.`
            );
        } else if(err.type==='unavailable-id'){
            setTimeout(()=>joinGame(code),150); return;
        } else {
...
function handleGuestData(d){
    switch(d.type){
        case 'WELCOME':
            console.log('[Guest] ✅ WELCOME, slot:', d.slotIndex);
            clearTimeout(joinTimer); stopConnecting();
            mySlotIndex=d.slotIndex;
            players=(d.players||[]).map(p=>p?{...p}:null);
            $('display-room-code').textContent=(d.roomCode||roomCode).toUpperCase();
            renderLobby(); break;
        ...
        case 'START_RACE':     prepareRace(d.text,d.players,mySlotIndex); break;
        case 'PROGRESS':
            if(players[d.slotIndex]){players[d.slotIndex].wpm=d.wpm;players[d.slotIndex].progress=d.progress;players[d.slotIndex].accuracy=d.accuracy;}
            updateTracks(); break;
        ...
        case 'ERROR':    stopConnecting(); alert(d.msg||'Room error.'); resetGame(); break;
    }
}

// ── Reset ────────────────────────────────────────────────────────────────────
function resetGame(){
    clearInterval(raceTimer); clearTimeout(cdTimer); clearTimeout(joinTimer); clearInterval(dotTimer);
    destroyPeer();
...
document.addEventListener('DOMContentLoaded',()=>{
    ...
    $('btn-join').addEventListener('click',()=>{
        const raw=($('input-room-code').value||'').trim();
        const code=raw.replace(/[^a-zA-Z]/g,'');
        if(code.length!==4){alert('Enter a 4-letter room code (letters only).');return;}
        joinGame(code);
    });

    $('input-room-code').addEventListener('input',function(){
        // Letters only, show uppercase, store as typed (normalised in joinGame)
