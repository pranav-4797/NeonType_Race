...
// PeerJS server config — using the official PeerJS cloud server explicitly
// The default 0.peerjs.com sometimes silently fails; explicit config is more reliable
...
// ─── Peer ID strategy ─────────────────────────────────────────────────────────
// IMPORTANT: PeerJS peer IDs must be short, lowercase alphanumeric + hyphens only.
// Host peer ID = "race-" + roomCode (e.g. "race-gjv9")
// Guest peer ID = "g-" + timestamp + random
// Room codes = 4 lowercase letters only (no numbers, no caps) — avoids server rejections

function makeRoomCode() {
    // Only lowercase letters a-z, avoiding l (looks like 1) and o (looks like 0)
...
const TEXTS = [
    ...
    `Rain has a way of erasing the boundaries between things. Puddles reflect neon signs in shattered fragments, making the wet street look like a mosaic of broken light. Pedestrians become silhouettes with glowing edges, their umbrellas like black mushrooms pushing through a luminous underworld. The rhythm of the rain on the pavement is the city own heartbeat, steady and relentless, punctuated by the hiss of tires and the distant thunder that rolls between towers. On nights like this, even the most hardened city dweller pauses, looks up, and remembers they are beautifully small.`,

    `Typing is a skill that rewards patience above all else. The fingers that move fastest are rarely the ones that started fastest. They are the ones that built their speed slowly, deliberately, through thousands of hours of careful repetition. Every character is a micro-decision about which finger, which motion, which fraction of a second to use. Speed emerges from the elimination of hesitation, and hesitation fades only when the body knows the keyboard so well that thought and motion become the same event. Practice is the slow accumulation of mastery, one keystroke at a time.`,
    ...
    `Networks are not built from cables alone. They are built from trust, the quiet agreement between machines that the data passing between them is worth protecting, routing, and delivering intact. Every packet carries a tiny flag that says it came from somewhere real and is going somewhere real. The engineers who designed these protocols in sparse academic offices never imagined the volume that would one day travel through their inventions. Yet the architecture holds, because it was built on sound principles: redundancy, error correction, and the assumption that every message must get through.`,
...
function calcLimit(text) { return Math.ceil(text.trim().split(/\s+/).length / IDEAL_WPM * 60 * TIME_FACTOR); }
...
function renderLobby() {
    ...
    } else { setStatus(`Joined · ${active}/${MAX_PLAYERS} Players`); }
}
...
function _createHostPeer() {
    const pid = hostPeerId(roomCode);
    console.log('[Host] Creating peer, ID:', pid);

    peer = new Peer(pid, makePeerConfig());

    peer.on('open', id => {
        console.log('[Host] ✅ Peer open. ID:', id, '| Room:', roomCode);
    ...
    peer.on('error', err => {
        ...
        if(err.type==='unavailable-id'){
            if(hostRetries<3){
                hostRetries++;
                roomCode=makeRoomCode();
                console.log('[Host] ID taken, retrying with new code:', roomCode);
                setTimeout(_createHostPeer, 500);
            } else {
                lockBtns(false);
                alert('Could not create room after several attempts. Please refresh and try again.');
                resetGame();
            }
...
function joinGame(code) {
    ...
    console.log('[Guest] Creating peer, ID:', gid, '| Joining room:', code);

    peer = new Peer(gid, makePeerConfig());

    peer.on('open', id => {
        console.log('[Guest] ✅ Peer open. ID:', id, '— connecting to host...');
        ...
        joinTimer=setTimeout(()=>{
            if(mySlotIndex===-1){
                alert(
                    `Could not connect to room "${code.toUpperCase()}".\n\n` +
                    `Check that:\n`+
                    `✓ The room code is exactly 4 letters\n`+
                    `✓ The host's browser tab is still open\n`+
                    `✓ You are both using the same website URL\n\n`+
                    `Ask the host to close and reopen the room, then try again.`
                );
                resetGame();
            }
        }, JOIN_TIMEOUT_MS);

        // Connect using the host's deterministic peer ID
        ...
        conn.on('close', ()  => { clearTimeout(joinTimer); if(mySlotIndex!==-1){alert('Host disconnected.');resetGame();} });
        conn.on('error', err => { console.error('[Guest] conn error:',err);clearTimeout(joinTimer);alert('Connection to host failed. Please try again.');resetGame(); });
    });
    ...
    peer.on('error',err=>{
        ...
        clearTimeout(joinTimer);lockBtns(false);
        if(err.type==='peer-unavailable'){
            alert(
                `Room "${code.toUpperCase()}" was not found on the server.\n\n`+
                `Please check:\n`+
                `✓ The 4-letter code is correct (case does not matter)\n`+
                `✓ The host's browser tab is still open\n`+
                `✓ Both of you opened the same URL: ${location.hostname}\n\n`+
                `Ask the host to create a new room if the issue continues.`
            );
        } else if(err.type==='unavailable-id'){
            // Guest ID collision — retry instantly
            setTimeout(()=>joinGame(code),100);return;
        } else if(err.type==='network'||err.type==='server-error'||err.type==='socket-error'){
...
function handleGuestData(d) {
    switch(d.type){
        case 'WELCOME':
            console.log('[Guest] ✅ WELCOME received. My slot:', d.slotIndex);
            clearTimeout(joinTimer);
...
document.addEventListener('DOMContentLoaded',()=>{
    ...
    $('input-room-code').addEventListener('input',function(){
        // Allow letters only, auto uppercase display but store lowercase
    ...
    $('btn-copy-code').addEventListener('click',()=>{
        navigator.clipboard.writeText(roomCode.toUpperCase()).catch(()=>{
            const t=document.createElement('textarea');t.value=roomCode.toUpperCase();
